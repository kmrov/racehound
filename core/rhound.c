...
/* If nonzero, %esp/%rsp-based memory accesses will also be processed. */
static int with_stack = 0;

/* If nonzero, locked memory accesses will also be processed. */
static int with_locked = 1;
/* ====================================================================== */
		    ...
		    int *size /* Out */, int *is_write /* Out */)
{
	...
	if (!is_tracked_memory_access(
		&insn, &at, with_stack, with_locked)) {
		return NULL;
	}
...
get_insn_info(struct insn *insn, void *data)
{
	...
	if (is_tracked_memory_access(insn, NULL, with_stack, with_locked)) {
		++fdata->num_insns;
